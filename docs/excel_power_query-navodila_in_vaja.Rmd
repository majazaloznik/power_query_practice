...
2. Odpri nov Excel fajl in s pomočjo spodnjih navodil vzpostavi podatkovni povezavi z obema viroma podatkov. (Skoraj 100% boš imel podatke z decimalkami, zato ne nujno preberi razdelek \@ref(dec) o manjkajočih decimalnih vejicah!)
...
Lahko pa to transformacijo uporabiš tudi, kadar želiš spremeniti ali odstraniti samo del besedila, npr. če želiš iz "Skupaj dejavnosti" spremeniti v "Skupaj", potem lahko v okence *Value to find* vneseš "dejavnosti" in *Replace with* pustiš prazno. 
...
- klikni na gumb *Custom Column* in izberi ime novega stolpca in v okence vnesi funkcijo `Text.Remove`, tako kot je prikazano na sliki spodaj. 
...
Ko potrdiš merge, se bo v tabeli pojavil not stolpec z ikonico z dvema puščicama. Na to ikonico klikneš in izbereš katere stolpce iz druge tabele želiš prikazane (če ne želiš vseh). 
